...
<html>
	...
	<body>
		...
		while(rs2.next()){
			...
			String date = rs2.getString(4);//入校时间
			...
			if(month.trim().equals("all")){
				if(year2.trim().equals(year)){
					if(rs2.getString(5).trim().equals("尚未离校")){
						number++;
						%>
						<form method="post" action="admin_deleteCarAction">
							<table border="1" width="100%">
						         <tr>
						           ...
						           <td width="10%"><%=rs2.getString(2) %></td>
						           <td width="20%"><%=rs2.getString(3) %></td>
						           <td width="20%"><%=rs2.getString(4) %></td>
						           <td width="20%"><%=rs2.getString(5) %></td>
						           <td width="10%"><%=rs2.getString(6) %></td>
						           <td width="10%">
						           		<input type="hidden" name="cardnumber" value=<%=rs2.getString(1) %>>
						           		<input type="hidden" name="outTime" value=<%=rs2.getString(5) %>>
						           </td>
						         </tr>   
					         </table>
						</form>         
